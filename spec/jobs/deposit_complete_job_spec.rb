...
RSpec.describe DepositCompleteJob do
  ...
  before do
    ...
    allow(Synchronizers::Work).to receive(:call)
    allow(Synchronizers::Collection).to receive(:call)
  ...
  context 'when a work' do
    ...
    it 'syncs, refreshes, and returns ack' do
      ...
      expect(Synchronizers::Work).to have_received(:call).with(work: object, cocina_object: dro_fixture, raise: false)
  ...
  context 'when a collection' do
    ...
    it 'syncs, refreshes, and returns ack' do
      expect(job.work(message)).to eq(:ack)
      expect(Synchronizers::Collection).to have_received(:call).with(collection: object,
                                                                     cocina_object: collection_fixture)
      expect(Turbo::StreamsChannel).to have_received(:broadcast_refresh_to).with(object)
    end
  end
