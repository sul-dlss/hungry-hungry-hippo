...
RSpec.describe WorkBuilder::PublicationDateBuilder do
  ...
  context 'when event date is EDTF with year only' do
    let(:event) do
      Generators::Description.event(type: 'publication', date: '2021')
    end
  ...
  context 'when event date is EDTF with year and month only' do
    let(:event) do
      Generators::Description.event(type: 'publication', date: '2021-03')
    end
  ...
  context 'when event date is EDTF' do
    let(:event) do
      Generators::Description.event(type: 'publication', date: '2021-03-05')
    end
  ...
  context 'when event date is not EDTF' do
    let(:event) do
      Generators::Description.event(type: 'publication', date: '2021-03-05', date_encoding_code: 'iso8601')
    end
