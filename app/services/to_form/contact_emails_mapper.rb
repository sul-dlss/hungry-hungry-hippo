...
module ToForm
  ...
  class ContactEmailsMapper < BaseMapper
    def initialize(cocina_object:, deposits_contact_email: nil)
      @cocina_object = cocina_object
      @deposits_contact_email = deposits_contact_email
      super(cocina_object:)
    end
    ...
    def call
      ...
      cocina_object.description.access.accessContact.filter_map do |access_contact|
        next if access_contact.value.blank? || access_contact.value == deposits_contact_email
    ...
    attr_reader :deposits_contact_email
  end
end
