...
class WorkPresenter < ApplicationPresenter
  def initialize(work:, work_form:)
    @work = work
    @work_form = work_form
    super()
  end

  attr_reader :work, :work_form

  delegate :status_message, :editable?, to: :status_presenter
  delegate :collection, :created_at, :druid, :user, to: :work
  delegate :abstract, :contact_emails_attributes, :keywords_attributes, :license, :other_work_subtype,
           :related_links_attributes, :related_works_attributes, :title, :version, :work_subtypes, :work_type,
           to: :work_form
  ...
  def status
    Sdr::Repository.status(druid:)
  end

  def status_presenter
    StatusPresenter.new(status:)
  end
end
