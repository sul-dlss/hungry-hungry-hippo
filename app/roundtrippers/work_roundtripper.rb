...
class WorkRoundtripper
  ...
  def normalized_original_cocina_object
    # Perform general normalization (e.g., lock) and DRO-specific normalization for legacy H2.
    @normalized_original_cocina_object ||= RoundtripSupport.normalize_cocina_object(
      cocina_object: LegacyDroNormalizer.call(cocina_object: @original_cocina_object, document: content.document?)
    )
  end
end
